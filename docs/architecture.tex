...
q_{dyn} &= \frac{1}{2}\rho_{air}\|v_{ned}\|^2 + n_q.
\end{align}
...
Vehicle parameters are declared in \texttt{src/vehicle/model/*.py}.
...
\begin{itemize}
    \item \texttt{IrisQuadForceModel} in \texttt{src/vehicle/forces/mc\_iris.py}: active rotor thrust and reaction torque,
    \item \texttt{PassiveSphereAeroForceModel} in \texttt{src/vehicle/forces/generic/passive\_sphere\_aero.py}: passive drag.
\end{itemize}
...
\subsubsection*{Force and Moment Models}
The X8 model uses \texttt{WingX8ForceModel} in \texttt{src/vehicle/forces/fw\_x8.py}. It computes aerodynamic forces from angle of attack/sideslip and aerodynamic derivatives, then adds propulsion force and moment.
...
The TS04 model uses:
\begin{itemize}
    \item \texttt{TS04ForceModel} in \texttt{src/vehicle/forces/vtol\_ts04.py}: active motor thrust, reaction torque, and gyroscopic terms,
    \item \texttt{TS04BlendedPassiveAeroForceModel} in \texttt{src/vehicle/forces/vtol\_ts04.py}: blend of sphere drag and wing-like passive aerodynamics.
\end{itemize}

\section{Transfer Alignment (External UDP Coupling)}
\texttt{src/transfer\_alignment.py} provides master/slave UDP transport with sequence-stamped state packets and a rigid-arm state transform from master body to slave body.
...
\subsubsection*{Flow chart (ASCII)}
{\footnotesize
\begin{verbatim}
...
\subsubsection*{Lockstep Message Sequence (ASCII)}
{\footnotesize
\begin{verbatim}
...
\clearpage
\section{Program Architecture (ASCII)}
\begin{verbatim}
                          +------------------------------+
                          |            PX4 SITL          |
...
\begin{verbatim}
python src/transfer_alignment.py --help
\end{verbatim}
...
from vehicle.model import IrisParameters, build_iris_force_models

P = IrisParameters()
world = World(parameters=P, force_models=build_iris_force_models(),
              vehicle_model="iris")
