...
Attitude dynamics and integration:
\begin{align}
...
\end{bmatrix}.
\end{equation}
with quaternion normalization after each integration step.
...
\texttt{IrisParameters} defines mass, inertia, motor, and passive drag terms. For improved alignment with the jMAVSim multicopter style, inertia and rotor time constant are set to:
\begin{itemize}
    \item $J_x=0.005$, $J_y=0.005$, $J_z=0.009$ kg m$^2$,
    \item motor time constant $\tau_m=0.005$ s.
\end{itemize}
...
$\sigma_{gyro}$ & gyroscope noise density & $2\cdot35/3600$ (code converts) & deg/s/\(\sqrt{\text{Hz}}\) \\
$rw_{acc}$ & accelerometer bias random walk & $6.0\times10^{-3}$ & m/s$^2\sqrt{\text{Hz}}$ \\
$rw_{gyro}$ & gyroscope bias random walk & $2\cdot4/3600$ (code converts) & deg/s\(\sqrt{\text{Hz}}\) \\
...
\paragraph{Characteristic values (current parameterization)}
All symbols in the table are defined in the equations above, with $f_{mag}=1/\Delta t$ and $rw_{mag}$ used to build the discrete innovation variance $\sigma_{b,d}$.
\begin{center}
\begin{tabular}{llll}
\toprule
Variable & Description & Value & Unit \\
...
\tilde{D}=p_d + \nu_D + \beta_D.
\end{align}
Here $w_i(k)$ is the zero-mean stochastic driving term (Gaussian random walk excitation) for the GPS bias state on axis $i$.
...
\subsection{Transfer Alignment (External UDP Coupling)}
...
Let $m$ be master body, $s$ slave body, and $w$ world (NED). The arm vector $r_{mb}$ is constant in master body coordinates. The relative orientation $q_{sm}$ (slave-from-master) is constant during coupling.

\paragraph{2) Slave pose from master pose}
\begin{align}
q_m &= \operatorname{norm}(y_m[3:7]), \qquad R_{mw}=R(q_m)^T, \\
r_w &= R_{mw}r_{mb}, \\
p_s &= p_m + r_w, \\
q_s &= \operatorname{norm}(q_{sm}\otimes q_m).
\end{align}
...
\begin{align}
\omega_w &= R_{mw}\omega_{m,b}, \\
v_{m,w} &= R_{mw}v_{m,b}, \\
v_{s,w} &= v_{m,w} + \omega_w\times r_w.
\end{align}
...
\begin{align}
a_{m,w} &= R_{mw}(\dot{v}_{m,b}+\omega_{m,b}\times v_{m,b}), \\
\alpha_w &= R_{mw}\dot{\omega}_{m,b}, \\
a_{s,w} &= a_{m,w} + \alpha_w\times r_w + \omega_w\times(\omega_w\times r_w).
\end{align}
...
\begin{align}
R_{sw} &= R(q_s), \qquad R_{sm}=R(q_{sm}), \\
v_{s,b} &= R_{sw}v_{s,w}, \\
\omega_{s,b} &= R_{sm}\omega_{m,b}, \\
\dot{v}_{s,b} &= R_{sw}a_{s,w} - \omega_{s,b}\times v_{s,b}, \\
\dot{\omega}_{s,b} &= R_{sm}\dot{\omega}_{m,b}, \\
\dot{q}_s &= \frac{1}{2}\Omega(\omega_{s,b})q_s.
\end{align}
...
\begin{itemize}
    ...
    \item Relative attitude $q_{sm}$ is constant during the coupled phase.
\end{itemize}
