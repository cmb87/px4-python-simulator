...
\begin{itemize}
    ...
    \item vehicle model parameterizations (\texttt{x8}, \texttt{iris}, \texttt{ts04}),
...
The rigid-body kernel in \texttt{src/vehicle/dynamics.py} evolves the 13-state vector under the summed body-frame force and moment input and gravity.
...
When catapult start is enabled, \texttt{World} temporarily replaces free 6DOF with a rail-constrained kernel from \texttt{src/vehicle/dynamics.py} (function \texttt{rail\_dynamics}), while catapult pull force is computed in \texttt{src/vehicle/vehicles/common\_forces/catapult\_rail.py}. Let $\hat r_{rail}^{ned}$ be the unit rail direction and $p_{rail,0}^{ned}$ the rail start point.
...
| lockstep scheduler | role: standalone/master/slave | MAVLink endpoint   |
+--------------------+-------------------------------+---------------------+
                     |
                     | controls_to_u()
                     v
...
Sensor synthesis is implemented in \texttt{src/vehicle/sensors/sensors.py}. Noise is enabled by default for IMU, magnetometer, GPS, and barometer.

\subsection{IMU (Accelerometer + Gyroscope)}
The IMU model in \texttt{src/vehicle/sensors/imu.py} converts state and state derivative into specific-force and angular-rate measurements with LPF, bias dynamics, and additive noise.
...
The magnetometer model in \texttt{src/vehicle/sensors/magnetometer.py} rotates Earth magnetic field from NED to body frame and applies calibration and stochastic terms.
...
The barometer model in \texttt{src/vehicle/sensors/barometer.py} converts simulated altitude into pressure with ISA relations and adds drift and noise.
...
\subsection{GPS}
The GPS model in \texttt{src/vehicle/sensors/gps.py} perturbs local NED position and velocity with random walk and white noise, then reprojects position to geodetic coordinates around the configured origin.
...
Vehicle parameters are declared per vehicle in \texttt{src/vehicle/vehicles/*/parameters.py}.
...
\begin{itemize}
    \item \texttt{IrisQuadForceModel} in \texttt{src/vehicle/vehicles/iris/forces.py}: active rotor thrust and reaction torque,
    \item \texttt{PassiveSphereAeroForceModel} in \texttt{src/vehicle/vehicles/common\_forces/passive\_sphere\_aero.py}: passive drag.
\end{itemize}
...
\subsubsection*{Force and Moment Models}
The X8 model uses \texttt{WingX8ForceModel} in \texttt{src/vehicle/vehicles/x8/forces.py}. It computes aerodynamic forces from angle of attack/sideslip and aerodynamic derivatives, then adds propulsion force and moment.

\subsection{TS04 (VTOL)}
\texttt{TS04Parameters} defines tilted rotor geometry and hybrid passive aerodynamics (sphere + wing blend).

\subsubsection*{Force and Moment Models}
The TS04 model uses:
\begin{itemize}
    \item \texttt{TS04ForceModel} in \texttt{src/vehicle/vehicles/ts04/forces.py}: active motor thrust, reaction torque, and gyroscopic terms,
    \item \texttt{TS04BlendedPassiveAeroForceModel} in \texttt{src/vehicle/vehicles/ts04/forces.py}: blend of sphere drag and wing-like passive aerodynamics.
\end{itemize}

\subsection{How to Add a New Vehicle}
Add a new folder under \texttt{src/vehicle/vehicles/} (for example \texttt{my\_uav/}) with:
\begin{itemize}
...
\end{itemize}
Then register the vehicle in \texttt{src/vehicle/vehicle\_catalog.py} by adding one \texttt{VEHICLES} entry.
...
\section{Transfer Alignment (External UDP Coupling)}
\texttt{src/vehicle/transfer\_alignment.py} provides master/slave UDP transport with sequence-stamped state packets and a rigid-arm state transform from master body to slave body.

\paragraph{Goal}
The goal of transfer alignment is to initialize and drive the slave vehicle from master truth during the coupled phase, so both platforms stay kinematically consistent before cutover. This provides smooth handover to local slave dynamics without large transients in pose, velocity, or attitude rates.

\subsubsection*{Readable Derivation}
The transfer maps a master rigid-body state to a slave state attached through a fixed arm and fixed relative attitude.

\paragraph{1) Frames and constants}
Let superscript $M$ denote master, superscript $S$ denote slave, and subscript $w$ denote world (NED). The rigid arm vector $r_b^{M}$ is constant in master body coordinates. The relative orientation $q^{SM}$ (slave-from-master) is constant during coupling.
In practice, small mounting misalignment can be represented as
\begin{equation}
q^{SM} = q_{nom}^{SM} \otimes \delta q, \quad \|\delta\theta\| \ll 1,
\end{equation}
where $q_{nom}^{SM}$ is the nominal calibration and $\delta q$ captures residual alignment error.
Here $\otimes$ denotes quaternion multiplication (Hamilton product), i.e., composition of rotations.

\paragraph{2) Slave pose from master pose}
\begin{align}
q^{M} &= \operatorname{norm}(y^{M}[3:7]), \qquad R_{Mw}=R(q^{M})^T, \\
r_w &= R_{Mw}r_b^{M}, \\
p^{S} &= p^{M} + r_w, \\
q^{S} &= \operatorname{norm}(q^{SM}\otimes q^{M}).
\end{align}
...
\paragraph{3) Slave velocity from rigid-arm kinematics}
\begin{align}
\omega_w^{M} &= R_{Mw}\omega_b^{M}, \\
v_w^{M} &= R_{Mw}v_b^{M}, \\
v_w^{S} &= v_w^{M} + \omega_w^{M}\times r_w.
\end{align}
The extra term $\omega\times r$ is the velocity induced at the arm tip by body rotation.

\paragraph{4) Slave acceleration from rigid-arm kinematics}
\begin{align}
a_w^{M} &= R_{Mw}(\dot{v}_b^{M}+\omega_b^{M}\times v_b^{M}), \\
\alpha_w^{M} &= R_{Mw}\dot{\omega}_b^{M}, \\
a_w^{S} &= a_w^{M} + \alpha_w^{M}\times r_w + \omega_w^{M}\times(\omega_w^{M}\times r_w).
\end{align}
The two arm terms are tangential acceleration $\alpha\times r$ and centripetal acceleration $\omega\times(\omega\times r)$.

\paragraph{5) Dynamics-ready slave body derivatives}
\begin{align}
R_{Sw} &= R(q^{S}), \qquad R_{SM}=R(q^{SM}), \\
v_b^{S} &= R_{Sw}v_w^{S}, \\
\omega_b^{S} &= R_{SM}\omega_b^{M}, \\
\dot{v}_b^{S} &= R_{Sw}a_w^{S} - \omega_b^{S}\times v_b^{S}, \\
\dot{\omega}_b^{S} &= R_{SM}\dot{\omega}_b^{M}, \\
\dot{q}^{S} &= \frac{1}{2}\Omega(\omega_b^{S})q^{S}.
\end{align}
These are exactly the quantities consumed by \texttt{world.observe\_external\_state(...)} in slave-coupled mode.

\subsubsection*{Rigid Arm Assumptions}
\begin{itemize}
    \item The lever arm between master and slave is rigid and fixed in the master body frame.
    \item There is no structural flex, no hinge, and no compliance in the coupling.
    \item Slave translational kinematics are induced by master rotation exactly through rigid-body terms $\omega\times r$ and $\alpha\times r + \omega\times(\omega\times r)$.
    \item Relative attitude $q^{SM}$ is constant during the coupled phase.
\end{itemize}
...
\subsubsection*{Cutover Trigger Modes and Configuration}
Cutover logic is active in \texttt{SIM\_ROLE=slave}. The mode is selected with
\texttt{SIM\_TRANSFER\_CUTOVER\_MODE} and supports exactly three values:
\begin{itemize}
    \item \texttt{never}: slave remains transfer-coupled forever; local slave dynamics are not entered automatically.
    \item \texttt{time}: cutover occurs when simulated transfer time satisfies
    \begin{equation}
    t_{us} \ge \texttt{SIM\_TRANSFER\_CUTOVER\_TIME\_S} \cdot 10^6.
    \end{equation}
    \item \texttt{mavlink\_cmd}: cutover occurs when a \texttt{COMMAND\_LONG} with command id \texttt{MAV\_CMD\_TRANSFER\_CUTOVER} is received (implemented as \texttt{MAV\_CMD\_USER\_1}, default id 31000).
\end{itemize}
...
After cutover (for \texttt{time} or \texttt{mavlink\_cmd}), the slave applies the last transformed state, synchronizes simulation time, and switches from transfer overwrite to its own local dynamics update path.
...
\texttt{src/visualizer/websockerPublisher.py} streams JSON ground-truth state for external visualizers and analysis tools.
...
\begin{itemize}
    ...
    \item UDP transfer alignment via \texttt{src/transfer\_alignment.py}.
    \item Ground-truth WebSocket via \texttt{src/visualizer/websockerPublisher.py}.
\end{itemize}
