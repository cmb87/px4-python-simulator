...
def mavlink_listener(name, connection_string):
    ...
    while True:
        ...
        if msg:
            print(f"[{name}] {msg.get_type()}: {msg.to_dict()}")
...
def Update(t, y, ydot, z, u, wind, P):
    ...
    drone['i_vel__cm/s']                = 65535 # round((   np.linalg.norm(z['gps'][3:6])    )*100)
    ...
    drone['f_abs_pressure__hPa']        = float((   95598       + z["barometer"]        )*0.01)
    drone['f_diff_pressure__hPa']       = float((   z["barometer"]        )*0.01)
...
def mavlink_simulation(name, connection_string):
    ...
    while True:
        ...
        if t__us % 52000 == 0:
            ...
            cog                 = 65535 # drone['i_cog__cdeg']      # Course over ground (NOT heading, but direction of movement), 0.0..359.99 degrees. If unknown, set to: 65535 [cdeg] (type:uint16_t)
